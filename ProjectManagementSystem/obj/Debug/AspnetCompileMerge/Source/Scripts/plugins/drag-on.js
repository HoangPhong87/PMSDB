...
$(function () {
    ...
    $.extend({
        DragOn: function (S, opt) { /* Scroll mechanics */
            var def = {
                exclusion: { 'input': '', 'textarea': '', 'select': '', 'object': '', 'iframe': '', 'details': '', 'id': '#gmap,#map-canvas,.button,[data-overflow=no-dragon],a > :not(img),.text-overflow' },
            	cursor : 'all-scroll',
            ...
            _this = {
                ...
                //},
                scrollParent: function (Sto, w) {
                    while (Sto && Sto[0] && S[0] != Sto[0] && ((Sto[0].nodeType != 1 || Sto.css('overflow') == 'visible' || Sto.hasClass('short-text') || Sto.hasClass('text-overflow')) || Sto[0].tagName.toLowerCase() == 'a' || (!w && Sto.data('overflow') == 'no-dragon'))) Sto = Sto.parent();
                    return Sto;
                },
